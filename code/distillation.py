...
from data_utils.distill_datasets_rationale import DistillDatasetRationale
...
def prepare_dataset(args, distiller):
    ...
    if args.dataset == 'raw':
        ...
        if args.do_train:
            data["train"] = DistillDatasetRationale(
                args, "train_rationale", distiller.student_tokenizer,
                distiller.teacher_tokenizers
            )
            log_rank("Num of train data: {}".format(len(data["train"])))
            
            data["dev"] = DistillDatasetRationale(
                args, "dev", distiller.student_tokenizer,
                distiller.teacher_tokenizers
            )
            log_rank("Num of dev data: {}".format(len(data["dev"])))
            ...
            if os.path.exists(os.path.join(args.data_dir, "test.jsonl")):
                data["test"] = DistillDatasetRationale(
                    args, "test", distiller.student_tokenizer,
                    distiller.teacher_tokenizers
                )
                log_rank("Num of test data: {}".format(len(data["test"])))
...
def finetune(
    ...
    for epoch in range(args.num_epochs):
        ...
        log_rank("train | epoch {:0>3d} | epoch_time={:.2f}s ({:0>2}:{:0>2}:{:0>2})".format(
            epoch + 1,
            epoch_seconds,
            int(epoch_seconds // 3600),
            int(epoch_seconds % 3600 // 60),
            int(epoch_seconds % 60),
        ))
...
def evaluate(args, tokenizer, model, dataset, split, device, repeat_times=None):
    ...
    # if eval_info["token_num"] == 0:
    #     eval_info["loss"] += 1  
    #     eval_info["token_acc"] += 1
    #     eval_info["top1_prob"] += 1
    #     print("Warning: token_num is zero, assigning default values to avoid division by zero.")
    # else:
